...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/30 16:15:02 by kmorunov          #+#    #+#             */
/*   Updated: 2022/10/30 17:02:24 by jmehlig          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float	ft_convert(int deci_places, char *tmp, char *tmp2, int fd)
{
	float	result;
	long	power;
	int		int1;
	...
	if (!ft_string_digit(tmp2))
	{
		ft_free(tmp);
		ft_free(tmp2);
		msg_error("Float input is wrong", NULL, fd);
	}
	int1 = ft_atoi(tmp);
	int2 = ft_atoi(tmp2);
...
float	ft_stof(char *str, int fd)
{
	...
	return (ft_convert(ft_strlen(tmp2), tmp, tmp2, fd));
}
